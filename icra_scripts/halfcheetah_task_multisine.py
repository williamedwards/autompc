...
@memory.cache
def gen_trajs(num_trajs=1000, traj_len=1000, seed=42, init_obs=None, task=None):
    rng = np.random.default_rng(seed)
    return multisine_generate(halfcheetah, task, halfcheetah_dynamics, rng, 
            init_min=init_obs, init_max=init_obs, n_freqs=20,
            traj_len=traj_len, n_trajs=num_trajs)

def halfcheetah_task_multisine():
    system = halfcheetah
    env = gym.make("HalfCheetah-v2")
    ...
    gen_sysid_trajs = lambda seed, n_trajs=500: gen_trajs(n_trajs, 200, seed=seed, init_obs=init_obs, task=task)
    gen_surr_trajs = lambda seed, n_trajs=500: gen_trajs(n_trajs, 200,  seed=seed, init_obs=init_obs, task=task)
