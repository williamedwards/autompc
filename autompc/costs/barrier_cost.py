        ...
        for boundedObs in self.obsConfiguration:
            ...
            if lower > -np.inf:
                if lower >= obs[index]:
                    ...
                    sum = sum + scale * 1/(-lower + obs[index])
            if upper < np.inf:
                if obs[index] >= upper:
                    sum += np.inf
                else:
                    sum = sum + scale * 1/(upper - obs[index])           
        return sum
        ...
        for boundedObs in self.obsConfiguration:
            ...
            if lower > -np.inf:
                if lower >= obs[index]:
                    ...
                    jacobian[index] += -scale / (-lower + obs[index])**2
            if upper < np.inf:
                if obs[index] >= upper:
                    jacobian[index] += np.inf
                else:
                    jacobian[index] += scale / (upper - obs[index])**2   
        ...
        for boundedObs in self.obsConfiguration:
            ...
            if lower > -np.inf:
                if lower >= obs[index]:
                    ...
                    hessian[index][index] += scale*2 / ((-lower + obs[index])**3)
            if upper < np.inf:
                if obs[index] >= upper:
                    hessian[index][index] += np.inf
                else:
                    hessian[index][index] += scale*2 / ((upper - obs[index])**3)
