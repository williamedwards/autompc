...
class QuadLimitCostFactory(CostFactory):
    """
    ...
    def __call__(self, cfg, task, trajs):
        Q = self._get_matrix(cfg, "Q", self._Q_fixed, self.system.observations)
        R = self._get_matrix(cfg, "R", self._R_fixed, self.system.controls)
        F = self._get_matrix(cfg, "F", self._F_fixed, self.system.observations)
        goal = self._get_goal(cfg, task)

        boundedStates = self._get_boundedState_barrier(cfg, "LogBarrier", self._scale_fixed)

        return QuadLimitCost(self.system, boundedStates, Q, R, F, goal=goal)
    ...
    def _get_boundedState_barrier(self, cfg, label, fixed_dict):
        boundedStates = dict()
        for name in (self.system.controls + self.system.observations):
            if name in fixed_dict:
                limit, upper = self._limits[name]
