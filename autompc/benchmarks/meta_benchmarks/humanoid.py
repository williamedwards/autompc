...
def humanoid_dynamics(env, x, u, n_frames=5):
    """
    x: traj[j-1].obs[:]
    u: action
    """
...
def gen_trajs(env, system, num_trajs=1000, traj_len=1000, seed=42):
    ...
    for i in range(num_trajs):
        ...
        for j in range(1, traj_len):
            ...
            obs = humanoid_dynamics(env, traj[j-1].obs[:], action)
            traj[j].obs[:] = obs
        trajs.append(traj)
    return trajs


class HumanoidBenchmark(Benchmark):

    def __init__(self, name = "Humanoid-v2", data_gen_method="uniform_random"):
    ...
    def dynamics(self, x, u):
        return humanoid_dynamics(self.env,x,u)
