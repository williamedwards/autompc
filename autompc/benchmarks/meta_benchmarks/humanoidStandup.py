...
def gen_trajs(env, system, num_trajs=1000, traj_len=1000, seed=42):
    ...
    for i in range(num_trajs):
        ...
        for j in range(1, traj_len):
            ...
            obs = humanoidStandup_dynamics(env, traj[j-1].obs[:], action)
            traj[j].obs[:] = obs
        trajs.append(traj)
    return trajs


class HumanoidStandupBenchmark(Benchmark):

    def __init__(self, name = "HumanoidStandup-v2", data_gen_method="uniform_random"):
    ...
    def dynamics(self, x, u):
        return humanoidStandup_dynamics(self.env,x,u)
