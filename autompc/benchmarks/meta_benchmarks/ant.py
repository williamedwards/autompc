...
def gen_trajs(env, system, num_trajs=1000, traj_len=1000, seed=42):
    ...
    for i in range(num_trajs):
        ...
        new_init_obs = np.concatenate([[0,0], init_obs[:27]])
        ...
        for j in range(1, traj_len):
            ...
            obs = ant_dynamics(env, traj[j-1].obs[:], action)
            traj[j].obs[:] = obs
        trajs.append(traj)
    return trajs


class AntBenchmark(Benchmark):

    def __init__(self, name = "Ant-v2", data_gen_method="uniform_random"):
    ...
    def dynamics(self, x, u):
        return ant_dynamics(self.env,x,u)
