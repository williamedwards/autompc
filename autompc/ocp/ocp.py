...
class OCP:
    ...
    def is_feasible(self, traj : Trajectory) -> bool:
        ...
        for i in range(len(traj.obs)):
            if np.any(traj.obs[i] < self._obs_bounds[:,0]) or np.any(traj.obs[i] > self._obs_bounds[:,1]): return False
        for i in range(len(traj.ctrls)):
            if np.any(traj.ctrls[i] < self._ctrl_bounds[:,0]) or np.any(traj.ctrls[i] > self._ctrl_bounds[:,1]): return False
        return True
