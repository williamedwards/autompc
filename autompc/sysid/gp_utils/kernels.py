...
class RBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if Y is None:
            ...
            Xs = np.tile(X / length_scale, (1, Y.shape[0], 1))
            Ys = np.tile(Y.T / length_scale, (X.shape[0], 1, 1))
            Ys = Ys.swapaxes(1,2)
            #Xs = np.vstack([X.T / length_scale] * Y.shape[0]).T
            #Ys = np.vstack([Y.T / length_scale] * X.shape[0])
