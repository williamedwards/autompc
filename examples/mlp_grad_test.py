...
gp2 = ampc.make_model(cartpole, MLP, cfg)
...
#ctrl = np.ones(1,)
ctrl = np.zeros(1,)
xs, state_jacs, ctrl_jacs = gp.pred_diff_parallel(np.vstack([state, state]), 
        np.vstack([ctrl, ctrl]))
x = xs[1,:]
state_jac = state_jacs[1,:]
ctrl_jac = ctrl_jacs[1,:]
state_jac2 = fd_jac(lambda y: gp2.pred_parallel(np.vstack([y,y]), 
