...
def dt_cartpole_dynamics(y,u,dt,g=9.8,m=1,L=1,b=1.0):
    yu = np.concatenate([y,u])
    dy = dt*cartpole_simp_dynamics(yu,g,m,L,b)
    return dy


class NoisyCartpoleModel(Model):
    def __init__(self, system, rng, noise_factor=0.1):
    ...
    def pred(self, state, ctrl):
        dy = dt_cartpole_dynamics(state, ctrl, self.dt)
        noise = self.rng.normal(1, self.noise_factor, state.shape)
        xpred = state + dy*noise
        return xpred
